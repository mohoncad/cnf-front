...
const headCells = () => [
  ...
  },
  {
    ...
    visible:
      Number(
       APPUAP.GetModulePermissions(APP.CONFIG.MODULE[14])
          .CustomerAmountView
      ) === 1,
  },
  {
...
function ImportBillListTable(props) {
  ...
  const [viewPermission, setViewPermission] = React.useState(
    Number(
      APPUAP.GetModulePermissions(APP.CONFIG.MODULE[14]).View
    ) === 1
  );
  const [deletePermission, setDeletePermission] = React.useState(
    Number(
      APPUAP.GetModulePermissions(APP.CONFIG.MODULE[14]).Delete
    ) === 1
  );
  const [editPermission, setEditPermission] = React.useState(
    Number(
      APPUAP.GetModulePermissions(APP.CONFIG.MODULE[14]).Edit
    ) === 1
  );
  const [restorePermission, setRestore] = React.useState(
    Number(
      APPUAP.GetModulePermissions(APP.CONFIG.MODULE[14]).Restore
    ) === 1
  );
...
class Screen extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      AddPermission:
        Number(
          new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[14]).Add
        ) === 1,
      SearchQuery: "",
   ...
   */
  async TryBillDelete(BillID) {
    this.__SetFAP("general", "Loading", "top", "center");
    await axios
      .delete(APP.ENV.URL.API.ROOT + "/company/import/delete", {
        headers: {
   ...
   */
  async TryBillRestore(BillID) {
    this.__SetFAP("general", "Loading", "top", "center");
    await axios
      .put(
        APP.ENV.URL.API.ROOT + "/company/import/restore",
        {
          id: BillID,
        },
        {
    ...
    await axios
      ...
      .then((response) => {
        ...
        if (response.data.success) {
          ...
          let $DataGridRows = [];
          let $GridData = $ResponseData.import_bill_list;
  ...
  handleFetchSearchResult(e) {
    ...
    let $SearchQuery =
      document.forms["ImportBillListSearchForm"]["search_query"].value.trim();
  ...
  handlePrintAction(type, id) {
    ...
    window.open(`report/${id}/${type}/import`);
  }
      ...
      <div>
        <Grid container spacing={0}>
          <Grid item xs={8} md={11}>
            <h2 style={{ margin: "0 0 20px 0" }}>Import Bill List</h2>
          </Grid>
          <Grid item xs={4} md={1}>
...
const ImportBillScreen = connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(Screen));

export { ImportBillScreen as Screen };
