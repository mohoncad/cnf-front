    ...
    await axios
      .get(
        APP.ENV.URL.API.ROOT +
          "/company/import/getUpdatedCode?client_id=" +
          client.id,
        {
          headers: {
      ...
      .then((response) => {
        if (response.data.success) {
          let code = `IMB_${client.CodePrefix}${response.data.importbill_newCode}`;
          handleBillCode(code);
        }
      });
  }
...
function Particulars(props) {
  ...
  const addToParticulars = () => {
    ...
    let payload = {
      ...
      actual_amount: stringToNum(parseFloat(voucherAmount), 2),
  ...
  const computeTotal = (tempArray) => {
    const total = tempArray.reduce((acc, currVal) => {
      if (currVal.customer_amount === "") {
        return acc + 0;
      }
      return acc + parseFloat(currVal.customer_amount);
    }, 0.0);
                              ...
                              {hasActualAmount && (
                                <TableCell align="left">
                                  <TextField
                                    ...
                                    onBlur={(event) => handleBlurVouc(event, index)}
                                    onChange={(event) =>
                              ...
                              {hasCustomerAmount && (
                                <TableCell align="left">
                                  <TextField
                                    ...
                                    onBlur={(event) => handleBlurCust(event, index)}
                                    onChange={(event) =>
