...
const createData = (sn, id, code, date, client, total, payment_type, deleted_by, deleted_at) => {
  return {
    sn,
    id,
    code,
...
function BillPaymentListTable(props) {
  ...
  const [viewPermission, setViewPermission] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[19]).View
    ) === 1
  );
  const [deletePermission, setDeletePermission] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[19]).Delete
    ) === 1
  );
  const [editPermission, setEditPermission] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[19]).Edit
    ) === 1
  );
  const [restorePermission, setRestore] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[19]).Restore
    ) === 1
  );
  const [deleteForever, setDeleteForever] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[19])
        .DeleteForever
    ) === 1
  );
...
BillPaymentListTable.propTypes = {
...
class Screen extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      AddPermission:
        Number(
          new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[19]).Add
        ) === 1,
      SearchQuery: "",
  ...
  OpenForm(FormMode, ActionId, CodeId) {
    if (!this.state.GridTrashMode) {
      ...
      this.props.func.HandlePaymentCode(CodeId);
      this.props.func.HandlePaymentId(ActionId);
    }
  }
  ...
  handleGridDeleteAction(SummaryID) {
    let title = this.state.GridTrashMode ? "Attention!" : "Delete Payment";
  ...
  TrySummaryDelete(SummaryID) {
    axios
      .delete(APP.ENV.URL.API.ROOT + "/company/BillPayment/delete", {
        headers: {
          Authorization: `Bearer ${Auth.getToken()}`,
        },
        data: {
          bill_pmt_id: SummaryID,
  ...
  TrySummaryRestore(SummaryID) {
    ...
    axios
      .put(
        APP.ENV.URL.API.ROOT + "/company/BillPayment/restore",
        {
          bill_pmt_id: SummaryID,
        },
        {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.success) {
          ...
          let $GridData = $ResponseData.bill_payment_list;
          ...
          for (let $i = 0; $i < $GridData.length; $i++) {
            ...
            let code = $GridData[$i].bill_pmt_code;
            let id = $GridData[$i].bill_pmt_id;
            let date = moment($GridData[$i].pmt_date).format("DD/MM/YYYY");
            let client = $GridData[$i].client_name;
            let total = $GridData[$i].total_amount;
            let payment_type = $GridData[$i].pmt_type;
          ...
          let TotalPages = Math.ceil(
            $ResponseData.total_bill_payment_list /
              this.state.DataGridRowsPerPage
          );
          ...
          this.setState({
            DataTotalRowsCount: $ResponseData.total_bill_payment_list,
      ...
      <div>
        <Grid container spacing={0}>
          <Grid item xs={8} sm={11}>
            <h2 style={{ margin: "0 0 20px 0" }}>Bill Payment List</h2>
          </Grid>
          <Grid item xs={4} sm={1}>
...
const mapDispatchToProps = (dispatch) => {
  return {
    func: {
      ...
      HandlePaymentId: (payload) => {
        dispatch(HandlePaymentId(payload));
      },
...
const BillPaymentScreen = connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(Screen));

export { BillPaymentScreen as Screen };
