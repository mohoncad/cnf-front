...
function ClientListTable(props) {
  ...
  const [viewPermission, setViewPermission] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[11]).View
    ) === 1
  );
  const [deletePermission, setDeletePermission] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[11]).Delete
    ) === 1
  );
  const [editPermission, setEditPermission] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[11]).Edit
    ) === 1
  );
  const [restorePermission, setRestore] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[11]).Restore
    ) === 1
  );
  const [deleteForever, setDeleteForever] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[11])
        .DeleteForever
    ) === 1
  );
...
class Screen extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      AddPermission:
        Number(
          new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[11]).Add
        ) === 1,
      SearchQuery: "",
  ...
  OpenForm(FormMode, ActionId, CodeId) {
    if (!this.state.GridTrashMode) {
      ...
      this.props.func.HandleClientId(ActionId);
  ...
  handleGridDeleteAction(ClientID) {
    let title = this.state.GridTrashMode ? "Attention!" : "Delete Client";
    let message = this.state.GridTrashMode
      ? "By pressing the OK button, this action can not be reversed! Are you sure to permanently delete this client?"
      : "Are you sure to delete this user?";
    this.__PD("DeleteUser", ClientID, title, message);
  }

  handleGridRestoreAction(ClientID) {
    this.__PD(
      "RestoreClient",
      ClientID,
      "Restore Client ",
      "Are you sure to restore this client?"
    );
  }
   ...
   */
  async TryUserDelete(ClientID) {
    this.__SetFAP("general", "Loading", "top", "center");
    await axios
      .delete(APP.ENV.URL.API.ROOT + "/company/clients/delete", {
        headers: {
   ...
   */
  async TryUserRestore(ClientID) {
    this.__SetFAP("general", "Loading", "top", "center");
    await axios
      .put(
        APP.ENV.URL.API.ROOT + "/company/clients/restore",
        {
          ClientID: ClientID,
        },
        {
  ...
  async GetGridList(FAPShow = true) {
    if (FAPShow) {
      this.__SetFAP("general", "Loading", "top", "center");
    }
    ...
    await axios
      ...
      .then((response) => {
        ...
        if (response.data.success) {
          ...
          let $DataGridRows = [];
          let $GridData = $ResponseData.client_list;
          ...
          for (let $i = 0; $i < $GridData.length; $i++) {
            ...
            console.log(code);
            let group = this.props.GlobalData.GroupList.filter(
              (data) => data.id === $GridData[$i].ClientGroupID
            )[0].Name;
            console.log(code);
            let name = $GridData[$i].Name;
            let codepre = $GridData[$i].CodePrefix;
            let mobile = $GridData[$i].Mobile;
            let registration = $GridData[$i].CreatedAt;
  ...
  async GetGroupList(FAPShow = true) {
    let url = APP.ENV.URL.API.ROOT + "/company/client-groups/free_list";
    ...
    await axios
      ...
      .then((response) => {
        if (response.data.success) {
          this.props.func.HandleGroupList(response.data.client_groups);
          this.GetGridList(FAPShow);
        }
      })
      .catch((error) => {
        this.__RemoveFAP();
  ...
  async getClientCode() {
    let url = APP.ENV.URL.API.ROOT + "/company/clients/getUpdatedCode";
    ...
    await axios
      ...
      .then((response) => {
        if (response.data.success) {
          this.props.func.HandleCode(response.data.client_newCode);
        }
      })
      .catch((error) => {
        this.__RemoveFAP();
  ...
  handleFetchSearchResult(e) {
    ...
    let $SearchQuery =
      document.forms["UserListSearchForm"]["search_query"].value.trim();
      ...
      <div>
        <Grid container spacing={0}>
          <Grid item xs={8} sm={11}>
            <h2 style={{ margin: "0 0 20px 0" }}>Clients List</h2>
          </Grid>
          <Grid item xs={4} sm={1}>
        ...
        >
          ...
          <ClientListTable
            groupList={this.props.GlobalData.GroupList}
...
const mapDispatchToProps = (dispatch) => {
  return {
    func: {
      ...
      HandleGroupList: (paylaod) => {
        dispatch(HandleGroupList(paylaod));
      },
