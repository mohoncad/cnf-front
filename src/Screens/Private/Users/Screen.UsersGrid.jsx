...
import {
    ...
    HandleBranchList, HandleContactNumber, HandleDataGridShouldReload, HandleEmail,
    HandleFormMode,
    HandleFormOpen, HandleFullName, HandleHasEditPermission,
    HandleIsActive,
    HandleIsMasterUser, HandleMasterFlagEditPermission, HandlePassword, HandleProfilePhotoFile, HandleProfilePhotoUrl,
    HandleUserCode,
    HandleUserID,
    HandleUserRoleID,
    HandleUserRoleList,
...
const createData = (sn, id, code, status, master_user, name, role, email, contact_number, deleted_by, deleted_at) => {
    return {sn, id, code, status, master_user, name, role, email, contact_number, deleted_by, deleted_at};
};
...
const headCells = [
    {id: 'sn', align: 'left', disablePadding: false, TrashModeOnly: false, label: '#'},
    {id: 'status', align: 'center', disablePadding: false, TrashModeOnly: false, label: 'Status'},
    {id: 'code', align: 'center', disablePadding: false, TrashModeOnly: false, label: 'Code'},
    {id: 'name', align: 'left', disablePadding: false, TrashModeOnly: false, label: 'Name'},
    {id: 'role', align: 'left', disablePadding: false, TrashModeOnly: false, label: 'Role'},
    {id: 'email', align: 'left', disablePadding: false, TrashModeOnly: false, label: 'Email'},
    {id: 'contact_number', align: 'left', disablePadding: false, TrashModeOnly: false, label: 'Contact Number'},
    {id: 'deleted_by', align: 'left', disablePadding: false, TrashModeOnly: true, label: 'Deleted By'},
    {id: 'deleted_at', align: 'left', disablePadding: false, TrashModeOnly: true, label: 'Deleted At'},
    {id: 'action', align: 'center', disablePadding: false, TrashModeOnly: false, label: 'Action'},
];
        ...
        <div className={classes.root}>
            <TableContainer onContextMenu={(e) => e.preventDefault()}>
                <Table size={'small'} className={classes.table}>
                    ...
                    <TableBody>
                        {stableSort(props.rows, getComparator(order, orderBy)).map((row, index) => {
                            const HasMasterUserRestriction = (Number(auth_user.IsMasterUser) === 0 && row.master_user);
                            const DeleteDisabled = Number(auth_user.id) === Number(row.id);
                            ...
                            return (
                                <TableRow key={row.id} hover
                                          onContextMenu={(e) => handleOpenContextMenu(e, {
                                              ...
                                              edit: () => {
                                                  props.onEditAction('EDIT', row.id);
                                                  handleCloseContextMenu();
                                              },
                                              delete: () => {
                                    ...
                                    <TableCell align="left" className={classes.TableCell}>{row.code}</TableCell>
                                    <TableCell align="left" className={classes.TableCell}>{row.name}</TableCell>
                                    <TableCell align="left" className={classes.TableCell}>{row.role}</TableCell>
                                    <TableCell align="left" className={classes.TableCell}>{row.email}</TableCell>
                                    <TableCell align="left"
                                               className={classes.TableCell}>{row.contact_number}</TableCell>
                                    ...
                                    <TableCell align="center" className={classes.TableCell}>
                                        ...
                                        {!props.trashMode && (
                                            <React.Fragment>
                                                <Tooltip title={"View"} placement={"top"} enterDelay={500}>
                                                    <IconButton
                                                        size={"small"}
                                                        disabled={HasMasterUserRestriction}
                                                        className={classes.TableActionButton}
                                                        onClick={() => props.onViewAction('VIEW', row.id)}>
                                                ...
                                                <Tooltip title={"Edit"} placement={"top"} enterDelay={500}>
                                                    <IconButton
                                                        ...
                                                        disabled={HasMasterUserRestriction}
                                                        className={classes.TableActionButton}
                                                        onClick={() => props.onEditAction('EDIT', row.id)}>
                                                ...
                                                <Tooltip title={"Delete"} placement={"top"} enterDelay={500}>
                                                    <IconButton
                                                        size={"small"}
                                                        disabled={HasMasterUserRestriction || DeleteDisabled}
                                        ...
                                        {props.trashMode && (
                                            <React.Fragment>
                                                <Tooltip title={"Restore"} placement={"top"} enterDelay={500}>
                                                    <IconButton
                                                        size={"small"}
                                                        className={classes.TableActionButton}
                ...
                <Menu
                    ...
                    {props.trashMode && (
                        <div>
                            <MenuItem onClick={ContextMenuAction.restore}>Restore</MenuItem>
                            <MenuItem onClick={ContextMenuAction.delete}>Delete Forever</MenuItem>
                        </div>
                    )}
                </Menu>
            </TableContainer>
...
class Screen extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            ModuleIndex: 1,
            TrashViewPermission: Number(new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[2]).Trash) === 1,
            GridTrashMode: false,
        ...
        this.handleSelectUserRoleID = this.handleSelectUserRoleID.bind(this);
        ...
        this.TryUserRestore = this.TryUserRestore.bind(this);

        this.TryUserDelete = this.TryUserDelete.bind(this);
        ...
        this.handleChangeGridRowsPerPage = this.handleChangeGridRowsPerPage.bind(this);
    ...
    OpenForm(FormMode, ActionId) {
        ...
        this.props.func.HandleUserID(ActionId);
    };
    ...
    handleSelectUserRoleID(RoleID) {
        this.setState({
            DataSelectedUserRoleID: RoleID,
        });
    }
    ...
    handleGridDeleteAction(UserID) {
        let title = this.state.GridTrashMode ? "Attention!" : "Delete User";
        let message = this.state.GridTrashMode ? "By pressing the OK button, this action can not be reversed! Are you sure to permanently delete this user?" : "Are you sure to delete this user?";
        this.__PD("DeleteUser", UserID, title, message);
    }


    handleGridRestoreAction(UserID) {
        this.__PD("RestoreUser", UserID, "Restore User ", "Are you sure to restore this user?");
    }
     ...
     */
    TryUserDelete(UserID) {
        this.__SetFAP("general", "Loading", "top", "center");
        axios.delete(APP.ENV.URL.API.ROOT + "/users/delete", {
            headers: {
                Authorization: `Bearer ${Auth.getToken()}`,
            },
            data: {
     ...
     */
    TryUserRestore(UserID) {
        this.__SetFAP("general", "Loading", "top", "center");
        axios.put(APP.ENV.URL.API.ROOT + "/users/restore", {
            UserID: UserID,
        }, {
            headers: {
                Authorization: `Bearer ${Auth.getToken()}`,
    ...
    GetGridList(FAPShow = true) {
         ...
         * @return array || object
         */
        let url = APP.ENV.URL.API.ROOT + '/users?row_limit=' + $row_limit + '&page=' + $current_page_number + '&search_query=' + $search_query + '&offset=' + $offset + '&trash_mode=' + this.state.GridTrashMode;

        if (this.state.GridFilterMode) {
            let Status = this.state.DataSelectedStatus,
                RoleID = this.state.DataSelectedUserRoleID;

            Status = (Status === 0) ? '' : (Status === 1) ? 1 : 0;
            RoleID = (RoleID === 0) ? '' : RoleID;

            url += '&status=' + Status + '&user_role_id=' + RoleID;
        }
        ...
        axios.get(url, {
            ...
            if (response.data.success) {
                ...
                let $DataGridRows = [];
                let $GridData = $ResponseData.GridList;
                ...
                for (let $i = 0; $i < $GridData.length; $i++) {
                    ...
                    let status = Number($GridData[$i].IsActive) === 1;
                    let master_user = Number($GridData[$i].IsMasterUser) === 1;
                    let name = $GridData[$i].FullName;
                    let role = $GridData[$i].UserRoleName;
                    let email = $GridData[$i].email;
                    let contact_number = $GridData[$i].ContactNumber;
                    let deleted_by = $GridData[$i].DeletedBy;
                    let deleted_at = $GridData[$i].DeletedAt;


                    let $DataObject = createData($sn_output, id, code, status, master_user, name, role, email, contact_number, deleted_by, deleted_at);
                ...
                let CurrentPageNo = Math.ceil(this.state.LastGridIndex / this.state.DataGridRowsPerPage);
                ...
                let TotalPages = Math.ceil($ResponseData.TotalRows / this.state.DataGridRowsPerPage);
                ...
                this.setState({
                    DataTotalRowsCount: $ResponseData.TotalRows,
    ...
    handleFetchSearchResult(e) {
        ...
        let $SearchQuery = document.forms['UserListSearchForm']['search_query'].value.trim();
    ...
    handleUpdateCurrentPage($PageNumber) {
        ...
        } else if ($PageNumber > this.state.DataGridCurrentPageNumber) // jump to the next selected page
        {
            nextRemainingTargetPage = ($PageNumber - this.state.DataGridCurrentPageNumber) - 1;
            finalOffset = this.state.LastGridIndex + (this.state.DataGridRowsPerPage * nextRemainingTargetPage);
            startIndex = finalOffset;
            //alert("Forward: "+finalOffset);
        } else if ($PageNumber < this.state.DataGridCurrentPageNumber) {
            nextRemainingTargetPage = (this.state.DataGridCurrentPageNumber - $PageNumber);
            finalOffset = this.state.FirstGridIndex - (this.state.DataGridRowsPerPage * nextRemainingTargetPage);
            startIndex = finalOffset;
    ...
    handleUpdateTmpPageToJump(e) {
        ...
        let $PageNumber = Number(document.forms['PageJumpForm']['PageNumber'].value);
        //alert("Go: -- PageNo: "+$PageNumber + " CurrentPage: "+this.state.DataGridCurrentPageNumber);
        if ($PageNumber !== this.state.DataGridCurrentPageNumber && $PageNumber > 0 && $PageNumber <= this.state.DataTotalPages) {
            //alert("Go");
            this.handleUpdateCurrentPage($PageNumber);
        } else {
            document.forms['PageJumpForm']['PageNumber'].value = this.state.DataGridCurrentPageNumber;
        }
    }
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if ((prevProps.GridShouldReload !== this.props.GridShouldReload) && this.props.GridShouldReload === true) {
            this.props.onUpdateGridShouldReload(false);
            this.GetGridList();
        }

        if (prevProps.GlobalData.DataGridShouldReload !== this.props.GlobalData.DataGridShouldReload && this.props.GlobalData.DataGridShouldReload === true) {
            this.GetGridList('', false);
            ...
            <div>
                <Grid container spacing={0}>
                    <Grid item xs={8} sm={11}>
                        <h2 style={{margin: "0 0 20px 0"}}>User List</h2>
                    </Grid>
                    <Grid item xs={4} sm={1}>
                ...
                <Paper variant={"elevation"} elevation={1}
                    ...
                    <Grid container spacing={0}>
                        <Grid item {...this.state.TrashViewPermission ? {xs: 8, sm: 11} : {xs: 10, sm: 11}}>
                            <form
                                ...
                                <Paper
                                    ...
                                    <InputBase
                                        ...
                                        autoFocus={true}
                                    />
                                    <IconButton type={'submit'} style={{padding: 7}} aria-label="Search">
                        ...
                        <Grid item {...this.state.TrashViewPermission ? {xs: 4, sm: 1} : {xs: 2, sm: 1}}>
                            <Paper
                                variant="outlined"
                                square={false}
                                style={{
                                ...
                                {this.state.TrashViewPermission && (
                                    <Tooltip title={this.state.GridTrashMode ? "Close Trash" : "View Trash"}
                                             placement="top">
                                        <IconButton style={{padding: 7, color: APP.CONFIG.COLORS.ACTION_BTN.DANGER}}
                                                    onClick={this.ToggleTrashMode}>
                                            {!this.state.GridTrashMode && (<DeleteSweepIcon fontSize={"small"}/>)}
                                            {this.state.GridTrashMode && (<CloseIcon fontSize={"small"}/>)}
                        ...
                        <Grid container spacing={1}>
                            ...
                            <Grid item xs={12} sm={2}>
                                <div
                                    ...
                                    <AutoCompleteSelect
                                        label={"User Role"}
                                        options={this.state.DataUserRoles}
                                        selectedValue={this.state.DataSelectedUserRoleID}
                                        ZeroIndexTitle={"All"}
                                        ZeroIndexDisabled={false}
                                        onChange={this.handleSelectUserRoleID}/>
                                </div>
                            </Grid>
                    ...
                    <UserListTable rows={this.state.DataGridRows} onEditAction={this.OpenForm}
                                   onViewAction={this.OpenForm} onDeleteAction={this.handleGridDeleteAction}
                                   onRestoreAction={this.handleGridRestoreAction} trashMode={this.state.GridTrashMode}/>
                </Paper>
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} sm={3}>
                        ...
                        <form name="PageJumpForm"
                              style={{display: "inline-block"}}
                              onSubmit={this.handleUpdateTmpPageToJump}>
                            <FormControl style={{marginLeft: "10px", marginRight: "15px", verticalAlign: "middle"}}>
                                <TextField
                    ...
                    <Grid item xs={12} sm={3}>
                        <div style={{display: "inline-block", verticalAlign: "middle", fontWeight: "bold"}}>Rows
                            Per Page
                        </div>
                        <FormControl style={{marginLeft: "10px", marginRight: "15px", verticalAlign: "middle"}}>
                            <Select
                    ...
                    <Grid item xs={12} sm={3} style={{textAlign: "right"}}>
                        ...
                        <IconButton
                            color={'primary'}
                            disabled={this.state.DataTotalRowsCount === this.state.LastGridIndex || this.state.DataGridNextPageNumber <= 0}
...
const UserListScreen = connect(mapStateToProps, mapDispatchToProps)(withRouter(Screen));

export {UserListScreen as Screen};
