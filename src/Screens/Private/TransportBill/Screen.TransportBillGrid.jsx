...
function TransportBillListTable(props) {
  ...
  const [viewPermission, setViewPermission] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[16]).View
    ) === 1
  );
  const [deletePermission, setDeletePermission] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[16]).Delete
    ) === 1
  );
  const [editPermission, setEditPermission] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[16]).Edit
    ) === 1
  );
  const [restorePermission, setRestore] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[16]).Restore
    ) === 1
  );
  const [deleteForever, setDeleteForever] = React.useState(
    Number(
      new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[16]).DeleteForever
    ) === 1
  );
  ...
  const handlePrint = (type, mode) => {
    ...
    props.onPrintAction(rowId, type);
  };
  const handleClose = () => {
    setAnchorEl(null);
  };
                    ...
                    <TableCell align="center" className={classes.TableCell}>
                      {!props.trashMode && (
                        <React.Fragment>
                          ...
                          >
                            <MenuItem
                              onClick={() => handlePrint("NORMAL", "PRINT")}
                            >
                              Print
                            </MenuItem>
                            <MenuItem
                              onClick={() => handlePrint("REGULAR", "PRINT")}
                            >
                              Print Regular
                            </MenuItem>
                          </Menu>
...
TransportBillListTable.propTypes = {
...
class Screen extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      TrashViewPermission:
        Number(
          new APP.SERVICES.UAP().GetModulePermissions(APP.CONFIG.MODULE[16])
            .Trash
        ) === 1,
      GridTrashMode: false,
    ...
    this.TryTransportRestore = this.TryTransportRestore.bind(this);

    this.TryTransportDelete = this.TryTransportDelete.bind(this);
  ...
  __PDRunAction() {
    ...
    if (action_name === "DeleteTransport") {
      this.TryTransportDelete(action_id);
    }

    if (action_name === "RestoreTransport") {
      this.TryTransportRestore(action_id);
    }
  }
  ...
  OpenForm(FormMode, ActionId, CodeId) {
    if(!this.state.GridTrashMode) {
      ...
      this.props.func.HandleTransportId(ActionId);
    }
  }
  ...
  handleGridDeleteAction(TransportID) {
    let title = this.state.GridTrashMode ? "Attention!" : "Delete Transport";
    ...
    this.__PD("DeleteTransport", TransportID, title, message);
  }

  handleGridRestoreAction(TransportID) {
    this.__PD(
      "RestoreTransport",
      TransportID,
      "Restore Transport ",
   ...
   */
  TryTransportDelete(TransportID) {
    this.__SetFAP("general", "Loading", "top", "center");
    axios
      .delete(APP.ENV.URL.API.ROOT + "/company/transport/delete", {
        headers: {
   ...
   */
  TryTransportRestore(TransportID) {
    this.__SetFAP("general", "Loading", "top", "center");
    axios
      .put(
        APP.ENV.URL.API.ROOT + "/company/transport/restore",
        {
          bill_id: TransportID,
        },
        {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.success) {
          ...
          let $GridData = $ResponseData.transport_bill_list;
          ...
          for (let $i = 0; $i < $GridData.length; $i++) {
            ...
            let id = $GridData[$i].bill_id;
            let code = $GridData[$i].bill_code;
            let date = moment($GridData[$i].CreatedAt).format("DD/MM/YYYY");
            let client = $GridData[$i].client_name;
            let total = $GridData[$i].total_amount;
            let advance = $GridData[$i].paid_amount;
            let due = $GridData[$i].due_amount;
  ...
  handleFetchSearchResult(e) {
    ...
    let $SearchQuery =
      document.forms["TransportListSearchForm"]["search_query"].value.trim();
  ...
  handlePrintAction(id, type) {
    window.open(`transport/report/${id}/${type}`);
  ...
  GetAll() {
    ...
    Promise.all([client, transportType, locationList])
      .then((res) => {
        const [tempClientList, transportType, location] = res;
        ...
        if(location.data.success) {
          this.props.func.HandleLocationList(
            location.data.location_dropdown_list
          )
        }
      })
      .catch((error) => {
      ...
      <div>
        <Grid container spacing={0}>
          <Grid item xs={8} sm={11}>
            <h2 style={{ margin: "0 0 20px 0" }}>Transport Bill List</h2>
          </Grid>
          <Grid item xs={4} sm={1}>
          {
              ...
              <Grid item xs={12} sm={2}>
                <div style={{ margin: "10px 0px 10px 0px" }}>
                  <AutoCompleteSelect
                    ...
                    options={this.state.DataTransportRoles}
                    selectedValue={this.state.DataSelectedTransportRoleID}
                    ZeroIndexTitle={"All"}
                    ZeroIndexDisabled={false}
                    onChange={this.handleSelectTransportRoleID}
                  />
                </div>
              </Grid>
...
const TransportBillScreen = connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(Screen));

export { TransportBillScreen as Screen };
